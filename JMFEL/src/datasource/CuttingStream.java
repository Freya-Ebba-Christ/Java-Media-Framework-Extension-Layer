 ...
 */
public class CuttingStream implements PushBufferStream, BufferTransferHandler {
    ...
    private boolean isRawAudio(Format fmt) {
        return (fmt instanceof AudioFormat) &&
                fmt.getEncoding().equalsIgnoreCase(AudioFormat.LINEAR);
    }
    
    public CuttingStream(TrackControl tc, PushBufferStream pbs, long start[], long end[]) {
        this.tc = tc;
        this.pbs = pbs;
        this.start = start;
        this.end = end;
